...
public class NoticeUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//파일 업로드 기능이 포함된 공지글 수정 처리용 컨트롤러
				...
				//3. 업로드할 파일의 저장 폴더 지정
				//저장 폴더가 현재 구동중인 웹컨테이너(WAS: 톰캣)에서
				...
				//객체 생성과 동시에 자동 파일이 업로드 됨 : 지정한 폴더에 자동 저장
				...
				if(originalFileName != null) {
					notice.setOriginalFliePath(originalFileName);
					...
					//전에 기록된 파일은 삭제처리함
					if(!mrequest.getParameter("ofile").equals(notice.getOriginalFliePath()))
						new File(savePath + "\\" + mrequest.getParameter("rfile")).delete();
				}else { //첨부 파일이 변경되지 않았을 때
					notice.setOriginalFliePath(mrequest.getParameter("ofile"));
