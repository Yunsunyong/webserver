...
<hr>
<h1>1. 서버로 보내는 값 없고, 결과로 문자열 받아 출력 처리하는 방법 : get 방식</h1>
<p id="p1" style="width:300px;height:50px;border:1px solid red;"></p>
<button id="test1">테스트</button>
<!-- jquery ajax 사용 1 -->
<script type="text/javascript">
	//jQuery(document).ready(function(){ //이벤트 처리 코드 작성 해야함});
	$(function(){
...
<script type="text/javascript">
	$(function(){
		$("#test2").click(function(){
			//전송방식이 get 일 때는
			//jQuery.get() 과 jQuery.ajax() 중 선택사용
			//$.get()사용시
			$.get("/testa/t2.do", 
...
<hr>
<h1>3. 서버로 여러 개의 값들을 보내고, 결과를 안 받는 경우 : post</h1>
...
<script type="text/javascript">
	$(function(){
		$("#test3").click(function(){
			...
			//$.post() 사용시
			$.post("/testa/t3.do", {
				title: $("#title").val(), writer: $("#writer").val(), content: $("#content").val()
			});
		...
		$("#test4").click(function(){
			...
			$.getJSON("/testa/t4.do", 
					...
					function(data){
						...
						$("#p4").text(data.no + ", " +data.userid + ", " + data.userpwd + ", "
			...
			$.ajax({
				...
				success: function(data){
					$("#p4").html($("#p4").text() + "<br>"
							+ data.no + ", " +data.userid + ", " + data.userpwd + ", "
							+ decodeURIComponent(data.username) + ", " + data.age + ", " + data.email + ", " + data.phone);
				}
			});
		});	//click
...
<hr>
<h2>5. 서버로 값 보내고, 결과로 list 나 map 을 json 배열로 받아 출력처리하는 방법 : get</h2>
			...
			$.ajax({
				...
				success: function(data){
					...
					for(var i in json.list){
						values += json.list[i].userid + ", "
						+ json.list[i].userpwd + ", "
						+ decodeURICompoment(json.list[i].username) + ", "
						+ json.list[i].age + ", "
						+ json.list[i].email + ", "
						+ json.list[i].phone + "<br>";
					}//for in
